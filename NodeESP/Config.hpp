...
struct functions_s
{ //Which Functions have been initialised successfully
        bool wifi = false;
        bool wifi_ap = false;
        bool wps = false;
...
struct connection_s
{ //Which Connetions should be established...
        bool ws;
        bool udp;
        bool mqtt;
} connection;
...
{
        if (SPIFFS.exists(con_file))
                SPIFFS.remove(con_file); // Delete existing file, otherwise the configuration is appended to the file
        ...
        delay(2000);
        if (reboot)
                ESP.restart(); //Mandatory. new values only on stack! will be loaded in json buffer
}
...
bool load_gen_set(JsonObject &root)
{
        if (root.containsKey("wifi_mode"))
                gen_set.wifi_mode = root["wifi_mode"];
