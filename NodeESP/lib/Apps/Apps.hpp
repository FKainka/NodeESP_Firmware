...
bool list_apps(JsonObject &msg, JsonObject &answ, bool &hasAnsw)
{
        ...
        for (int i = 0; i < len_app_lists; i++)
        {
                ...
                while (exc_app->app != "")
                {
                        ...
                        if (String(app_set.app_exec) == exc_app->app)
                                answ["app_running"] = exc_app->title + "<br>" + exc_app->info;
                        exc_app++;
                }
        }
        hasAnsw = true;
...
void exec_app(const char *app_c)
{
        ...
        for (int i = 0; i < len_app_lists; i++)
        {                                        //Go through lists of list of app (list_app_lists is only the list of all sub lists)
        ...
        if (!executed)
        {                              //App was not found -> look if it is an "inter app"
        ...
        if (!executed)
        { //if nothing was called at all load the dflt app
...
void task_ref(void *parameter)
{
        ...
        while (1)
        {
                ...
                if (hasAnsw)
                { //if called function had an answear -> send through defined communcation port defined in task parameter
                        if (!root.containsKey("cmd"))
                                root["cmd"] = para->functions[0].cmd;
                        if (para->con->udp)
...
void firmware_update()
{
        ...
        strcpy(app_set.app_exec, "dflt_init");
